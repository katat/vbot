...
module.exports = function() {
  this.cmdOptions = {
    ...
    include: {
      alias: 'i',
      describe: 'include specified scenario to test',
      show: true
    },
    imgdir: {
      alias: 'd',
  ...
  this.exec = function () {
    ...
    Object.keys(this.cmdOptions).forEach((opt) => {
      ...
      if (opt === 'host') {
        vbotOpts.host = val
        console.info(colors.prompt('> Override host target with ' + val))
      }
      if (opt === 'imgdir') {
        vbotOpts.imgdir = val
      }
    ...
    vbot.start().catch((err) => {
      console.log(colors.error(err.message))
    })
    vbot.on('scenario.start', (scenario) => {
      console.log(`> started scenario ${scenario.name}`.blue)
    })
    ...
    vbot.on('action.executed', (log) => {
      ...
      console.info(colors.data(`>>> duration:${log.duration/1000}s`))
      if (log.screenshot) {
        ...
        if (log.screenshot.analysis) {
          ...
          if (analysis.misMatchPercentage) {
            ...
            console.info(colors.warn(`>>>> diff: ${log.screenshot.files.diff}`))
          } else {
