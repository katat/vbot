...
class VBot extends EventEmitter {
  constructor (options = {mismatchThreshold: 0}) {
    ...
    this.options.imgdir = options.imgdir || `${process.cwd()}/vbot`
  }
      ...
      for (let i = 0; i < scenario.actions.length; i++) {
        ...
        if (action.shot) {
          let screenshot = await this.capture(action, i, imgFolder).catch((err) => {
            actionLog.screenshot = err.err || {err: err}
            return
          })
          if (screenshot) {
  ...
  async runSchema (schema) {
    schema.scenarios.forEach(async (scenario) => {
      ...
      if (!this.options.showWindow) {
        let disableAnimation = "(typeof jQuery !== 'undefined') && (jQuery.fx.off = true);" +
        'var css = document.createElement( "style" );' +
        'css.type = "text/css";' +
        'css.innerHTML = "* { transition: none !important; transform: none !important; animation-delay: 0.01s !important; transition-duration: 0.01s !important; animation: none !important; }";' +
        'document.body.appendChild( css );'
        await this.client.eval(disableAnimation)
        this.client.client.Animation.animationStarted((e) => {
          this.client.client.Animation.setPaused({animations: [e.animation.id], paused: true})
          this.client.client.Animation.releaseAnimations({animations: [e.animation.id]})
        })
      }
      // await this.client.client.Animation.setPlaybackRate({playbackRate: 100})
  ...
  getScreenshotFileName (action, index) {
    let filename = `${index}_${action.type}`
    if (action.selector) {
      filename += '-' + action.selector.replace(' ', '_')
    }
    return filename
  }
