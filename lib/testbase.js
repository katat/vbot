...
class VBot extends EventEmitter {
  ...
  async runActions (scenario, rebase) {
    ...
    return new Promise(async (resolve, reject) => {
      for (let i = 0; i < scenario.actions.length; i++) {
        ...
        console.info(`running action index:${i} type:${action.type} selector:${action.waitFor}`)
        if (action.waitFor) {
        ...
        if (action.shot) {
          let screenshot = await this.capture(action, i, imgFolder)
  ...
  async capture (action, stepIndex, folder) {
    return new Promise((resolve) => {
      let filename = `${stepIndex}_${action.type}-${action.waitFor.replace(' ', '_')}`
  ...
  async wait (action) {
    await this.client.wait(action.waitFor, action.waitTimeout)
  }
